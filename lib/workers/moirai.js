...
(function() {
  var Promise, auth, conf, emptyResolve, getOrCreateAsset, handleAddCluster, handleAddData, handleAddUser, handleRemoveCluster, handleRemoveUser, moirai, teams, users, _;
  ...
  moirai.getTeamKeys = function(team) {
    ...
    return users.get_users_by_name(allMemberNames, 'promise').then(function(userList) {
      var keyList;
      keyList = userList.rows.map(function(user) {
        return _.findWhere(user.doc.data.publicKeys || [], {
          name: 'moirai'
        });
      });
  ...
  getOrCreateAsset = function(assetData, team) {
    var clusters, existingClusterWithName, url, _ref;
    url = '/moirai/clusters';
    clusters = ((_ref = team.rsrcs.moirai) != null ? _ref.assets : void 0) || [];
