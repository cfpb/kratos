...
(function() {
  ...
  teams.get_team_details = function(team_ame, actor_name) {
    return Promise.all([teams.get_team(db, team_name, 'promise'), users.get_user(actor_name, 'promise')]).then(function(_arg) {
      ...
      _.forEach(team.rsrcs, function(resource_data, resource_name) {
        ...
        if (detailHandler) {
          return rsrcs_promises[resource_name] = detailHandler(resource_data.assets, team, actor).then(function(asset_details) {
            var assets, zipped_assets;
            zipped_assets = _.zip(resource_data.assets, asset_details);
            assets = _.map(zipped_assets, function(_arg1) {
  ...
  teams.handle_get_team_details = function(req, resp) {
    ...
    return teams.get_team_details(team_name, req.session.user).then(function(team_details) {
