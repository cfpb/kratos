...
update_audit_entries = (db, team_id, sync_status, callback) ->
    ...
    await
      for entry in unsynced_audit_entries
        ...
        for resource, i in resources
          resource.handle_team_event(entry, doc, defer(errs[entry.id][i]))

    sync_status = {}
    _.each(errs, (err_list, entry_id) ->
      err_list = _.compact(err_list)
      success = not err_list.length
      console.log('ENTRY ERROR:', err_list) if not success
      sync_status[entry_id] = success
    )
    await update_audit_entries(db, doc._id, sync_status, defer(err))
    # TODO handle err
