...
(function() {
  ...
  teams.add_asset = function(db, actor_name, team_name, resource, asset_data) {
    var user_promise, users;
    users = require('./users');
    if (actor_name === conf.COUCHDB.SYSTEM_USER) {
      user_promise = Promise.resolve({
        name: conf.COUCHDB.SYSTEM_USER,
        roles: []
      });
    } else {
      user_promise = users.get_user(actor_name, 'promise');
    }
    return Promise.all([teams.get_team(db, team_name, 'promise'), user_promise]).then(function(_arg) {
