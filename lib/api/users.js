...
(function() {
  var add_remove_role, conf, couch_utils, iced, isInt, user_db, users, utils, uuid, _, __iced_k, __iced_k_noop;
  ...
  users.get_users = function(callback) {
  ...
  users.handle_get_users = function(req, resp) {
    ...
    if (resource) {
      ...
      return users.get_users().pipe(resp);
    }
  };

  users.get_user = function(user_id, callback) {
    return couch_utils.rewrite(user_db, 'base', '/users/org.couchdb.user:' + user_id, callback);
  };

  users.handle_get_user = function(req, resp) {
    return users.get_user(req.params.user_id).pipe(resp);
  };

  add_remove_role = function(client, action_type, user, resource, role, callback) {
    var action;
    action = {
      action: action_type,
      key: resource,
  ...
  users.add_role = function(client, user, resource, role, callback) {
    return add_remove_role(client, 'a+', user, resource, role, callback);
  };

  users.remove_role = function(client, user, resource, role, callback) {
    return add_remove_role(client, 'a-', user, resource, role, callback);
  };

  users.handle_add_remove_role = function(action_type) {
  ...
  users.reactivate_user = function(client, user, callback) {
    var action;
    action = {
      action: 'u+',
      uuid: uuid.v4()
  ...
  users.deactivate_user = function(client, user, callback) {
    var action;
    action = {
      action: 'u-',
      uuid: uuid.v4()
  ...
  users.handle_add_user = function(req, resp) {
